...
@section('default')
    ...
    <form action="{{route('posts.update',$post->id)}}" method="post" class="w-50 m-auto">
        @csrf
        @method('patch')
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control" id="title" placeholder="Minecraft!" value="@if(old("title")){{ old("title")}}@else{{$post->title}}@endif">
            @error('title')
        ...
        <div class="mb-3">
            ...
            <textarea name="content" class="form-control" id="content" rows="3"
                      placeholder="I love this version of minecraft.">@if(old("title")){{ old("content")}}@else{{$post->content}}@endif</textarea>
            @error('content')
                <p class="text-danger">{{$message}}</p>
            @enderror
        </div>
